...
import Controller.findings_DBN as dbn


class my_DBNFindings:
    old_rate = None
    X = None
    Y = None
    pro = None
    ...
    def new_train_performance(self, data, root, modeltype, feature_type, classification_type, preprocess_status, preprocess_canvas, split_rate,variableDataset, old_split_rate = '20'):
        ...
        if split_rate.strip().isnumeric() == False:
            ...
            ttk.Label(root, text='DBN-Model:').place(relx = .885, rely = 0.1, anchor='n')
            ...
            if variable_model.get() == 'DBN':
                model = dbn.getDBN(feature_type, classification_type)
                open = deepFrame.deepView.create_DBN_frame( root, model['main'], feature_type, classification_type)
                open.place(relx = 0.21, rely = 0.03, anchor='n')
            elif variable_model.get() == 'CNN-DBN':
                    model = dbn.getCNN_DBN(feature_type, classification_type)
    ...
    def performance_frame(self, container, model, feature_type, classification_type, preprocess_status, preprocess_canvas, X_test, Y_test, axes):
        ...
        count = 2
        if self.current_model == 'DBN':
            performance_parameters , roc_plot, classification_matrix, clf_report = test_single_model(model['main'], flatten(X_test), Y_test, classification_type, axes)
        else:
            if self.current_model == 'CNN-DBN':  
                model['second'] = dbn.getDBN_CNN(feature_type, classification_type)   
        ...
        if 'second' in model.keys():
                frame1 = ttk.Frame(container, height= 300, width= container.winfo_width()/4 )
            
                # grid layout for the input frame
                frame1.columnconfigure(0, weight=10)
                frame1.columnconfigure(0, weight=10)
                
                ttk.Label(frame1, text=   'DBN Model Parameters:', font=( 10)).grid(column=0, row=4, sticky=tk.W)

                count = 5

                for i in model['second'].get_params():
                    ttk.Label(frame1, text= i + ': ').grid(column=0, row=count, sticky=tk.W)
                    ttk.Label(frame1, width=30, text = str(model['second'].get_params()[i]) ).grid(column=1, row=count, sticky=tk.W)
                    count = count +1

                for widget in frame1.winfo_children():
                    widget.grid(padx=5, pady=5)
                
                frame1.place(relx = 0.525, rely = 0.12, anchor='n')
    ...
    def load_preprocess(self, root, model, feature_type, classification_type, preprocess_status, preprocess_canvas , variable_dataset = 'CIDDS-001', split = '20', make4D = False):
        ...
        if self.current_model == 'CNN-DBN':
           # X_test = feature_extractor_DBN(model['main'], X_test)
