...
import View.myDBN_Findings as dbn
class intro_menu:
    data_canvas = None
    data_status = None
    data = None
    ...
    def Findings_Frame_input(self, container):
        ...
        tk.Button(frame, text='DBN Models', width = 40, command= (lambda: self.open_DBNfinding('DBN')))
...
#Frame for Deep and Shallow Selection
    ...
    #Frame for Deep and Shallow Selection
    def open_DBNfinding(self, modeltype):
       
        if self.data_canvas.itemcget(self.data_status, 'fill') == 'green':     
            dbn.my_DBNFindings(self.data, modeltype, self.feature_type, self.classification_type)
        else:
